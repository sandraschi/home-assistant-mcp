...
function LLMSettings() {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0f0f12]/80 p-5 space-y-3">
      ...
      <select
        className="w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
  ...
  return (
    <div className="space-y-6 py-4 max-w-4xl">
      ...
      <div className="rounded-2xl border border-white/10 bg-[#0f0f12]/80 p-5 space-y-3 text-sm text-slate-400">
        <p>
          <strong className="text-slate-300">HA_URL</strong> — Home Assistant URL (e.g.
          http://homeassistant.local:8123 or http://192.168.1.50:8123)
        </p>
        <p>
          <strong className="text-slate-300">HA_TOKEN</strong> — Long-Lived Access Token (Profile in
          HA → Long-Lived Access Tokens → Create)
        </p>
        <p>
          <strong className="text-slate-300">HA_MCP_PORT</strong> — Backend port (default 10796)
        </p>
      </div>
      <LLMSettings />
    </div>
